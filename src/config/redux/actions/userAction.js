...
export const registerWorker = (dataForm, navigate) => async (dispatch) => {
  try {
    ...
    const result = await axios.post(`${process.env.REACT_APP_URL_API}auth/regisworker`, dataForm);
...
export const registerEmployer = (dataForm, navigate) => async (dispatch) => {
  try {
    ...
    const result = await axios.post(`${process.env.REACT_APP_URL_API}auth/regisemp`, dataForm);
    const user = result.data;
    console.log(result.data);
